...
export function useDashboard() {
  ...
  async function fetchOverview() {
    try {
      const res = await requestClient.get('/dashboard/user/overview/')
      overview.value = res
    } catch (e) {
  ...
  async function fetchTopModels(limit = 5) {
    try {
      const res = await requestClient.get('/dashboard/user/top-models/', { params: { limit } })
      topModels.value = res || []
    } catch (e) {
  ...
  async function fetchRequestStats(days = 7) {
    try {
      const res = await requestClient.get('/dashboard/user/request-stats/', { params: { days } })
      requestStats.value = res || []
    } catch (e) {
  ...
  async function fetchInviteInfo() {
    try {
      const res = await requestClient.get('/users/invite/info/')
  ...
  async function fetchInviteRewards() {
    try {
      const res = await requestClient.get('/users/invite/rewards/')
