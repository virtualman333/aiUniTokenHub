...
- **分页参数只有一处解析**：`page` / `page_size` 的解析、`page_size` 的上限、以及「翻到第几页」的切法，全在 `apps/utils/pagination.py`。那行 `page = int(request.query_params.get('page', 1))` 曾经在本仓手抄 **11 遍**（`ai_models` 2、`api_proxy` 1、`dashboard` 3、`image_gen` 1、`tickets` 1、`users` 3），`page_size` 的默认值 `20` 也跟着抄了 11 遍；而 `settings.REST_FRAMEWORK['PAGE_SIZE']` 是第三份 —— 它**没有任何东西读它**（DRF 只在配了 `DEFAULT_PAGINATION_CLASS` 时才看这个键，本仓一个分页类都没配），改它分页行为一点不变。代价不只是重复：手抄的那几行**假设了输入的形状**，而输入完全由客户端控制，于是 `?page=abc` 抛 `ValueError`、`?page=0` 让负数切片落到 Django 上，两条都是 **500**；`?page_size=999999` 则一次把整张表拉回浏览器。现在非法输入统一回 400 加一句给人看的话、`page_size` 超上限夹到 `MAX_PAGE_SIZE`（夹紧而不是报错 —— 报错会打断「翻到最后一页时顺手放大 page_size」的老客户端）。顺带修掉 `/api/users/usage-logs/`：那个 viewset 没重写 `list`，走的是 DRF 默认实现 —— 裸数组、没有 `total`、也没有上限，前端只能写 `res.results || res || []` 去猜这次拿到的是哪种形状。`apps/utils/tests/test_pagination.py` 既锁行为（含一条反向对照：同一次请求只换 `REST_FRAMEWORK['PAGE_SIZE']`，结果必须不同），也锁来源（全仓剥注释后不许再出现手抄的那一行、`20` 在模块代码里只许有一处、登记过的模块必须真的在调分页）。
- **「响应统一走 `{code, msg, data}`」这句话现在有人扛了**：`router.register` 注册的视图里，DRF 会照基类**凭空补上**标准动作；子类不重写就等于在用 DRF 的默认实现 —— 那一定不是信封（`Response(serializer.data)` 给出裸对象，`list` 更是**裸数组**：没有 `total`、没有 `page_size` 上限）。这个形状本仓手工修过两次（`/api/users/usage-logs/`、`/api/models/upstream-accounts/`），**第三次是 `ModelCategoryViewSet`：它六个标准动作一个都没写**，而 `list` 是 `AllowAny` 的公开接口 —— 前端 `ModelManagement.vue` 因此写着 `res.results || res || []` 去猜这一次拿到的是哪种形状。现在这件事由 `apps/docs/tests/test_response_envelope.py` 扛：它用 `ast` 枚举所有 `router.register` 的视图（不启动 Django、不连数据库），现算「哪些标准动作是继承来的」，有一处没本地定义就必须**要么照同文件里的 `ModelProviderViewSet` 补一个、要么在 `ENVELOPE_DEBT` 里写明理由**。表是**债**不是豁免区，而且两向对账（收干净了还留着 = 僵尸条目，会红）；`list` 另有一条**零容忍**断言 —— 继承来的 `list` 就是公开接口上的裸数组，连登记都不给。这一层只回答「服务这个端点的是本仓写的代码，还是 DRF 的默认实现」；本仓自己写的 `@action` 里还有 **8 处**直接 `return Response(...)`，它们不会凭空出现，逐类记在同一张表的值里。
- **前端说「我要全部」必须靠翻页，不许靠一个更大的数字**：后端把 `page_size` 夹到 `MAX_PAGE_SIZE`（100）之后，前端原先 **5 处**用 `page_size: 1000` / `'9999'` 表达「我要全部」的地方就变成了**静默截断** —— 模型下拉少几项、接口记录导出少几千行，全程不报错。这两种写法在代码里长得几乎一样，只有在数据超过 100 条时才分岔，所以没法靠读代码发现。现在唯一入口是 `frontend/src/utils/pagination.ts` 的 `fetchAllPages()`：它按页拿到不满一页为止（拿不满一页就是到底了）、认裸数组（没分页的接口一次就是全部，不能去翻，否则同一批数据会被重复累加）、翻到 `MAX_PAGES` 还没拿全就把 `truncated` 置真。`PAGE_SIZE` 必须与后端 `MAX_PAGE_SIZE` 相等，**`truncated` 必须被说出来**（每处调用都要有 `if (truncated) { ElMessage.warning(...) }`）—— 不报出来的话用户看到的是一份少了几行的导出，和修之前一模一样。顺带修掉 `/api/models/upstream-accounts/`：这个 viewset 也没重写 `list`，走 DRF 默认实现，于是 `page` / `page_size` 一个都不解析、返回裸数组、没有任何上限，前端那行 `page_size: 1000` 在那里**完全无效**（不报错也不生效）；它的 queryset 还带一层 `Sum` 聚合，无上限地整表吐出去代价最难看。`apps/utils/tests/test_frontend_pagination_contract.py` 静态扫全仓 `page_size` 字面量（不许超过上限，并带一条正控证明正则没瞎）、两向对账在用 `fetchAllPages` 的文件、要求每处翻页都报截断；动态那部分用 node 的 `--experimental-strip-types` **把 `pagination.ts` 真跑起来**（这个文件零依赖，不需要构建）—— 因为循环终止条件写错的后果是**转圈转到天荒地老**而不是报错，正则看不出来，用例里那条「后端一直给满页」专盯这个，外面套 60 秒看门狗。
